...
export default function App() {
  ...
  async function autoSeedSemis() {
    ...
    const Aids = setOf(poolTeamsMap[A.id])
    const Bids = setOf(poolTeamsMap[B.id])

    // Include both pool and cross-pool phases; exclude bracket games
    const matchesFor = (idsSet) =>
      matches.filter(m => (m.phase === 'pool' || m.phase === 'cross_pool') &&
        (idsSet.has(m.team1_id) || idsSet.has(m.team2_id)))

    const standingsA = computeStandings({
      teams: teams.filter(t => Aids.has(t.id)),
      matches: matchesFor(Aids),
      sets: allSets,
    })
    const standingsB = computeStandings({
      teams: teams.filter(t => Bids.has(t.id)),
      matches: matchesFor(Bids),
      sets: allSets,
    })
