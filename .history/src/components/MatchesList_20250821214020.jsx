...
export default function MatchesList({ matches, selectedTid, isAdmin, onOpen, onDelete, pools, filterPhase, setFilterPhase, filterPool, setFilterPool }) {
  ...
  const hasCross = matches.some(m => m.phase === 'cross_pool')
  ...
  return (
    <Card>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <Select value={filterPhase} onChange={(e)=>setFilterPhase(e.target.value)} className="max-w-[200px]">
          <option value="all">All phases</option>
          <option value="pool">Pool</option>
          <option value="semifinal">Semifinal</option>
          <option value="final">Final</option>
          {hasCross && <option value="cross_pool">Cross‑pool</option>}
        </Select>
        {filterPhase==='pool' && (
            ...
            <li key={m.id} className="rounded-2xl border p-3 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs capitalize opacity-70">
                  {m.phase === 'cross_pool' ? 'Cross‑pool' : m.phase}
                  {m.pool_id ? ' • Pool' : ''} • {m.status}
                </div>
              </div>
              ...
              {filtered.map((m)=> (
                <tr key={m.id}>
                  ...
                  <td className="py-2 capitalize">
                    {m.phase === 'cross_pool' ? 'Cross‑pool' : m.phase}
                    {m.pool_id ? ` (${{...(pools.find(p=>p.id===m.pool_id)||{}).name||'Pool'}})` : ''}
