...
export default function PoolsPanel({
  ...
  // When provided, enables cross-pool creation UI
  ...
  // Local state for cross-pool manual creation
  const [cross, setCross] = useState({ sourcePool:'', sourceTeam:'', targetPool:'', when:'' })
    ...
    <Card>
          ...
          return (
            <div key={pool.id} className="rounded-2xl border p-4 space-y-3">
              ...
              <div className="rounded-xl border p-3 space-y-2">
                ...
                <p className="text-xs opacity-60">Pool matches count toward pool seeding when completed.</p>
              </div>
            </div>
          )
        })}
      ...
      {typeof onCreateCrossPoolMatches === 'function' && poolsSorted.length >= 2 && (
        <div className="mt-6 rounded-2xl border p-4 space-y-3">
          ...
          <p className="text-xs opacity-70">Pick a source team from one pool and a target pool. This will create one match against each team in the target pool.</p>
          ...
          <p className="text-xs opacity-60">Cross‑pool games are created with phase <code>cross_pool</code> and <b>do count</b> toward each team's own pool seeding.</p>
        </div>
      )}
    </Card>
  )
