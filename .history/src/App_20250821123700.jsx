...
export default function App() {
  ...
  const [poolTeamsMap, setPoolTeamsMap] = useState({}) // { poolId: [teamId, ...] }
  ...
  // When tournament changes, fetch everything scoped to it
  useEffect(() => {
  ...
  async function generateRoundRobin(pool_id) {
    if (!client) return
    const ids = (poolTeamsMap[pool_id] || [])
    if (ids.length < 2) return alert('Need at least 2 teams in the pool')
    const pairs = []
    for (let i=0;i<ids.length;i++) for (let j=i+1;j<ids.length;j++) pairs.push([ids[i], ids[j]])
    const payload = pairs.map(([a,b]) => ({
      tournament_id: selectedTid, team1_id: a, team2_id: b, scheduled_at: null, status: 'scheduled', phase: 'pool', pool_id
    }))
  ...
  // Bracket auto-seeding: A1 vs B2, B1 vs A2 (needs exactly 2 pools)
  async function autoSeedSemis() {
    ...
    // Ensure final exists (teams TBD for now)
  ...
  // Leaderboard (tournament-wide completed matches)
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6 pb-24 sm:pb-6">
        ...
        {selectedTid && (
          <PoolsPanel
            ...
            onGenerateRR={generateRoundRobin}
          />
        )}
