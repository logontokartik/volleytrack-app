...
function labelOrTBD(team) { return team?.name || "TBD"; }
...
async function createBracketPlaceholders() {
  if (!client || !selectedTid) { alert("Select a tournament first"); return; }
  ...
  for (let i = semis.length; i < 2; i++) {
    const { data: m, error } = await client
      ...
      .select("*")
      .single();
    if (!error && m) { await ensureSetsForMatch(m.id); created++; } else if (error) { alert(error.message); return; }
  }
  if (finals.length < 1) {
    const { data: m2, error: e2 } = await client
      ...
      .select("*")
      .single();
    if (!e2 && m2) { await ensureSetsForMatch(m2.id); created++; } else if (e2) { alert(e2.message); return; }
  }
...
async function assignMatchTeams(mid) {
  ...
  if (!t1 || !t2 || t1 === t2) { alert("Pick two different teams to assign"); return; }
...
async function autoSeedSemisFromPools() {
  try {
    if (!selectedTid) { alert("Select a tournament first"); return; }
    ...
    const entryFor = (letter) => {
      return leaderboardByPool.find(({pool}) => {
    ...
    const A = entryFor("A");
    const B = entryFor("B");
    if (!A || !B) { alert("Couldn't find Pool A and Pool B. Please name pools 'Pool A' and 'Pool B'."); return; }

    const A1 = A.standings?.[0], A2 = A.standings?.[1];
    const B1 = B.standings?.[0], B2 = B.standings?.[1];
    if (!A1 || !A2 || !B1 || !B2) { alert("Need top 2 from each pool to auto-seed."); return; }

    const semis = matches.filter(m => (m.stage||'').toLowerCase() === 'semi');
    if (semis.length < 2) { alert("Please generate two Semi-Final placeholders first."); return; }
    const [s1, s2] = semis.slice(0,2).sort((x,y)=> (x.id||0) - (y.id||0));
    ...
    for (const u of updates) {
      const { error } = await client.from("matches").update({ team1_id: u.team1_id, team2_id: u.team2_id }).eq("id", u.id);
      if (error) { alert(error.message); return; }
  ...
  const leaderboard = React.useMemo(() => {
    ...
    for (const m of matches) {
      ...
      if (map.has(m.team1_id)) { const r = map.get(m.team1_id); r.sw += sw1; r.sl += sw2; r.pf += pf1; r.pa += pf2; r.pd = r.pf - r.pa; }
      if (map.has(m.team2_id)) { const r = map.get(m.team2_id); r.sw += sw2; r.sl += sw1; r.pf += pf2; r.pa += pf1; r.pd = r.pf - r.pa; }
    }

    return Array.from(map.values()).sort((a,b)=> (b.w - a.w) || (b.sw - a.sw) || (b.pd - a.pd) || a.name.localeCompare(b.name));
  ...
  const leaderboardByPool = React.useMemo(() => {
    ...
    // Cross-pool matches COUNT toward each pool's teams (per your requirement).
    function computePoolTable(teamIdsSet) {
      ...
      for (const m of matches) {
        ...
        // Update stats ONLY for teams that are part of this pool
        if (teamIdsSet.has(m.team1_id)) {
          const r1 = map.get(m.team1_id);
          if (r1) { r1.sw += sw1; r1.sl += sw2; r1.pf += pf1; r1.pa += pf2; r1.pd = r1.pf - r1.pa; }
          if (winId === m.team1_id) r1 && (r1.w++);
          if (loseId === m.team1_id) r1 && (r1.l++);
        }
        if (teamIdsSet.has(m.team2_id)) {
          const r2 = map.get(m.team2_id);
          if (r2) { r2.sw += sw2; r2.sl += sw1; r2.pf += pf2; r2.pa += pf1; r2.pd = r2.pf - r2.pa; }
          if (winId === m.team2_id) r2 && (r2.w++);
          if (loseId === m.team2_id) r2 && (r2.l++);
        }
      }
      return Array.from(map.values()).sort((a,b)=> (b.w - a.w) || (b.sw - a.sw) || (b.pd - a.pd) || a.name.localeCompare(b.name));
    }
...
{selectedTid && (
  <Card>
    <div className="flex items-center justify-between mb-3">
      <h2 className="text-lg font-medium flex items-center gap-2"><Trophy className="w-4 h-4"/> Bracket</h2>
      {isAdmin && (
        <div className="flex gap-2">
          \1
                ...
                {semis.map((m,idx)=> {
                  ...
                  return (
                    <div key={m.id} className="rounded-2xl border p-3">
                      ...
                      <div className="mt-2 font-medium">{m.team1?.name || "TBD"} <span className="opacity-60">vs</span> {m.team2?.name || "TBD"}</div>
                      {isAdmin && (
                        <div className="grid sm:grid-cols-3 gap-2 mt-3">
                          <Select value={b.team1} onChange={(e)=>setBracketAssign(prev=>({...prev, [m.id]: {...(prev[m.id]||{}), team1: e.target.value}}))}>
                            <option value="">Team 1…</option>
                            {teams.filter(t=>allIds.has(t.id)).map(t=>(<option key={t.id} value={t.id}>{t.name}</option>))}
                          </Select>
                          <Select value={b.team2} onChange={(e)=>setBracketAssign(prev=>({...prev, [m.id]: {...(prev[m.id]||{}), team2: e.target.value}}))}>
                            <option value="">Team 2…</option>
                            {teams.filter(t=>allIds.has(t.id)).map(t=>(<option key={t.id} value={t.id}>{t.name}</option>))}
                          </Select>
                          <div className="flex gap-2">
                            <Button onClick={()=>assignMatchTeams(m.id)} className="bg-white">Save</Button>
                            <Button onClick={()=>clearMatchTeams(m.id)} className="bg-white">Clear</Button>
                          </div>
                        </div>
                      )}
                    </div>
