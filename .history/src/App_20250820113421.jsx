...
import { Plus, Volleyball, Upload, Database, Calendar, Users, Trophy, Save, Edit3, Trash2, RefreshCcw, LogIn, LogOut, Lock } from "lucide-react";

/*
  VolleyTrack — volleyball scoring app with public view + admin auth
  ------------------------------------------------------------------
  • Public visitors (no login) can view: Matches, Live scoreboards (read-only), Leaderboard
  • Single admin (Supabase Auth email/password) can: Add teams, Schedule matches, Update scores, Complete matches
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setMatches(m || []);
        const { data: s } = await client.from("sets").select("*").order("set_number");
        setAllSets(s || []);
      } finally {
        setLoading(false);
      }
  ...
  async function completeMatch() {
    if (!client || !selectedMatch) return;
    const { error } = await client.from("matches").update({ status: "completed" }).eq("id", selectedMatch.match.id);
    if (error) alert(error.message);
    await openMatch(selectedMatch.match.id);
    setRefresh((x) => x + 1);
  }
  ...
  // Leaderboard (computed client-side from completed matches)
  const leaderboard = React.useMemo(() => {
    const map = new Map();
    teams.forEach(t => map.set(t.id, { id: t.id, name: t.name, w: 0, l: 0 }));
    ...
    for (const m of matches) {
      ...
      if (t1===t2) continue; // ignore ties/invalid
      const winId = t1>t2 ? m.team1_id : m.team2_id;
      const loseId = t1>t2 ? m.team2_id : m.team1_id;
      if (map.has(winId)) map.get(winId).w++;
      if (map.has(loseId)) map.get(loseId).l++;
    }
    return Array.from(map.values()).sort((a,b)=> b.w - a.w || a.l - b.l || a.name.localeCompare(b.name));
  }, [teams, matches, allSets]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                ...
                {!leaderboard.length && (
                  <tr><td className="py-2 text-sm opacity-60" colSpan={4}>No completed matches yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        {/* Matches (Public can view; admin can edit) */}
        <Card>
          <div className="flex items-center justify-between mb-3">
          ...
          <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}>
            <Card>
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  {isAdmin && <Button onClick={completeMatch} className="bg-emerald-600 text-white">Mark Completed</Button>}
